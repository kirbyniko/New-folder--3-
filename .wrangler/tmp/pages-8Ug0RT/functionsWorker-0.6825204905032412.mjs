...
async function onRequest(context2) {
  ...
  try {
    ...
    params.push(limit);
    const { results: events } = await env2.DB.prepare(query).bind(...params).all();
    if (events.length > 0) {
...
async function onRequest8(context2) {
  ...
  try {
    ...
    for (const event of events) {
      const { results: bills } = await env2.DB.prepare(`
        SELECT 
          b.bill_number as number,
          b.title,
          b.url,
...
// ../.wrangler/tmp/pages-8Ug0RT/functionsRoutes-0.5554155749472987.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-c4La7E/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-c4La7E/middleware-loader.entry.ts
...
@neondatabase/serverless/index.mjs:
  ...
  *)
*/
//# sourceMappingURL=functionsWorker-0.6825204905032412.mjs.map
