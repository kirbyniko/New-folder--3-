...
// .wrangler/tmp/pages-za5GgL/functionsWorker-0.27186345723546657.mjs
...
function parseNeonUrl(connectionString) {
  ...
  const [, username, password, host, database] = match2;
  const projectId = host.split(".")[0].split("-").slice(-1)[0];
...
async function onRequest11(context22) {
  ...
  try {
    if (request.method === "GET" && !url.searchParams.get("id")) {
      ...
      let query2 = "SELECT * FROM scraper_templates WHERE is_public = 1";
      const params = [];
      if (category) {
        query2 += " AND category = ?";
      ...
      if (search) {
        query2 += " AND (name LIKE ? OR description LIKE ?)";
    ...
    if (request.method === "POST") {
      ...
      `).bind(
        ...
        data.is_public !== void 0 ? data.is_public : 1
      ).run();
      return new Response(JSON.stringify({
        success: true,
        id,
    ...
    if (request.method === "PUT" && url.searchParams.get("id")) {
      const id = url.searchParams.get("id");
      const data = await request.json();
      const selectorsJson = data.selectors ? JSON.stringify(data.selectors) : void 0;
      const updates = [];
      const params = [];
      if (data.name) {
        updates.push("name = ?");
      ...
      if (selectorsJson) {
        ...
        params.push(selectorsJson);
      }
      if (data.example_url !== void 0) {
        updates.push("example_url = ?");
        params.push(data.example_url);
      }
      if (data.requires_javascript !== void 0) {
        updates.push("requires_javascript = ?");
        params.push(data.requires_javascript);
      }
...
var routes = [
  ...
  {
    routePath: "/api/scraper-templates",
    mountPath: "/api",
    method: "",
    middlewares: [],
    modules: [onRequest11]
...
// .wrangler/tmp/pages-za5GgL/kbw44yop4ok.js
...
// .wrangler/tmp/bundle-gglcaH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-gglcaH/middleware-loader.entry.ts
...
@neondatabase/serverless/index.mjs:
  ...
  *)
*/
//# sourceMappingURL=kbw44yop4ok.js.map
