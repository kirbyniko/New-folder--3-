...
/**
 * Fast cached API endpoint for Congress meetings
...
export const handler: Handler = async (event) => {
  ...
  try {
    const cachePath = join(process.cwd(), 'public', 'cache', 'congress.json');
    ...
    if (existsSync(cachePath)) {
      ...
      console.log('⚠️ Cache is stale, falling back to live scraping');
    }

    // Fallback: Import and run original scraper
    console.log('🔄 Live scraping (no cache available)');
    const { handler: originalHandler } = await import('./congress-meetings');
    ...
    console.error('Error serving congress data:', error);
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({
