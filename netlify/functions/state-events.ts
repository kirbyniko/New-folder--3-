...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    // Special handling for Virginia, Arizona, Tennessee, Massachusetts, Indiana, Maryland & Missouri - use pre-scraped static data
    if (stateAbbr === 'VA' || stateAbbr === 'AZ' || stateAbbr === 'TN' || stateAbbr === 'MA' || stateAbbr === 'IN' || stateAbbr === 'MD' || stateAbbr === 'MO' || stateAbbr === 'WI') {
      const stateNames: Record<string, string> = { 'VA': 'Virginia', 'AZ': 'Arizona', 'TN': 'Tennessee', 'MA': 'Massachusetts', 'IN': 'Indiana', 'MD': 'Maryland', 'MO': 'Missouri', 'WI': 'Wisconsin' };
      const fileNames: Record<string, string> = { 'VA': 'virginia-events.json', 'AZ': 'arizona-events.json', 'TN': 'tennessee-events.json', 'MA': 'massachusetts-events.json', 'IN': 'indiana-events.json', 'MD': 'maryland-events.json', 'MO': 'missouri-events.json', 'WI': 'wisconsin-events.json' };
