...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    // Special handling for Virginia, Arizona, Tennessee, Massachusetts, Indiana, Maryland, Missouri, Wisconsin, Colorado, Minnesota, South Carolina, Alabama, Louisiana, Kentucky, Oregon & Oklahoma - use pre-scraped static data
    if (stateAbbr === 'VA' || stateAbbr === 'AZ' || stateAbbr === 'TN' || stateAbbr === 'MA' || stateAbbr === 'IN' || stateAbbr === 'MD' || stateAbbr === 'MO' || stateAbbr === 'WI' || stateAbbr === 'CO' || stateAbbr === 'MN' || stateAbbr === 'SC' || stateAbbr === 'AL' || stateAbbr === 'LA' || stateAbbr === 'KY' || stateAbbr === 'OR' || stateAbbr === 'OK') {
      const stateNames: Record<string, string> = { 'VA': 'Virginia', 'AZ': 'Arizona', 'TN': 'Tennessee', 'MA': 'Massachusetts', 'IN': 'Indiana', 'MD': 'Maryland', 'MO': 'Missouri', 'WI': 'Wisconsin', 'CO': 'Colorado', 'MN': 'Minnesota', 'SC': 'South Carolina', 'AL': 'Alabama', 'LA': 'Louisiana', 'KY': 'Kentucky', 'OR': 'Oregon', 'OK': 'Oklahoma' };
      const fileNames: Record<string, string> = { 'VA': 'virginia-events.json', 'AZ': 'arizona-events.json', 'TN': 'tennessee-events.json', 'MA': 'massachusetts-events.json', 'IN': 'indiana-events.json', 'MD': 'maryland-events.json', 'MO': 'missouri-events.json', 'WI': 'wisconsin-events.json', 'CO': 'colorado-events.json', 'MN': 'minnesota-events.json', 'SC': 'southcarolina-events.json', 'AL': 'alabama-events.json', 'LA': 'louisiana-events.json', 'KY': 'kentucky-events.json', 'OR': 'oregon-events.json', 'OK': 'oklahoma-events.json' };
