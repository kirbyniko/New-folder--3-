...
// ========================================
// BUILD TAB - CUSTOM SCRAPER BUILDER
// ========================================

document.getElementById('save-custom-scraper')?.addEventListener('click', () => {
  const name = document.getElementById('build-name').value.trim();
  const jurisdiction = document.getElementById('build-jurisdiction').value.trim();
  const level = document.getElementById('build-level').value;
  const stateCode = document.getElementById('build-state-code').value.trim().toUpperCase();
  const startUrl = document.getElementById('build-start-url').value.trim();
  const type = document.getElementById('build-type').value;
  
  const containerSelector = document.getElementById('build-container').value.trim();
  const titleSelector = document.getElementById('build-title').value.trim();
  const dateSelector = document.getElementById('build-date').value.trim();
  const linkSelector = document.getElementById('build-link').value.trim();
  
  // Validation
  if (!name || !jurisdiction || !startUrl) {
    showStatus('❌ Please fill in Name, Jurisdiction, and Start URL', 'error');
    return;
  }
  
  if (!containerSelector || !titleSelector || !dateSelector) {
    showStatus('❌ Please fill in Container, Title, and Date selectors', 'error');
    return;
  }
  ...
  const scraper = {
    ...
    jurisdiction,
    level,
    stateCode: stateCode || jurisdiction.substring(0, 2).toUpperCase(),
    startUrl,
    type,
    selectors: {
  ...
  showStatus(`✅ "${name}" saved to library!`, 'success');
  
  // Clear form
  document.getElementById('build-form').querySelectorAll('input[type="text"], input[type="url"]').forEach(input => {
    input.value = '';
  });
  
  // Switch to Library tab to view
  setTimeout(() => {
    document.querySelector('[data-tab="library"]').click();
  }, 1500);
});

document.getElementById('clear-build-form')?.addEventListener('click', () => {
  document.getElementById('build-form').querySelectorAll('input[type="text"], input[type="url"]').forEach(input => {
    input.value = '';
  });
...
document.getElementById('build-state-code')?.addEventListener('input', (e) => {
