...
async function runNDAndSD() {
  ...
  for (const state of states) {
    ...
    if (events.length > 0) {
        ...
        try {
          ...
          await pool.query(`
            INSERT INTO events (
              ...
              committee_name, scraper_source, fingerprint, scraped_at
      ...
      console.log(`  ℹ️  No events found (legislature may be in interim)`);
    }
  }
  
  await pool.end();
  console.log('\n🎉 All 50 states now have scrapers! (48/50 actively scraping, 2 in interim)');
}
